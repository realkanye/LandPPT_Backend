...
# Animation / transition / narration helpers from upstream ppt-master are
# intentionally NOT ported into LandPPT — call sites below check these for
# None and skip the corresponding features.
...
def create_pptx_with_native_svg(
    ...
    narration_audio: dict[str, Path] | None = None,  # Unused — kept for upstream API parity
    use_narration_timings: bool = False,              # Unused — kept for upstream API parity
    narration_padding: float = 0.5,                   # Unused — kept for upstream API parity
) -> bool:
    ...
    try:
        ...
        # Narration / audio variables retained as no-op for upstream code paths
        # that still reference them (kept empty so all branches are skipped).
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  <Relationship Id="rId1" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/slideLayout" Target="../slideLayouts/slideLayout1.xml"/>{extra_rels}
                ...
                # Narration support intentionally not ported into LandPPT.
