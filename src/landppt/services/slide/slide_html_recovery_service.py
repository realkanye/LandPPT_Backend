...
class SlideHtmlRecoveryService:
    ...
    async def _generate_html_with_retry(self, context: str, system_prompt: str, slide_data: Dict[str, Any], page_number: int, total_pages: int, max_retries: int=3) -> str:
        ...
        # 获取用户选择的模板，用于fallback
        selected_template = None
        # 优先使用已经加载到slide_data中的模板，避免重复查询数据库（防止长项目时数据库连接超时）
        if 'selected_template' in slide_data and slide_data['selected_template']:
            selected_template = slide_data['selected_template']
            logger.info(f'✅ [模板加载] 第{page_number}页使用预加载选中模板: ID={selected_template["id"]}, 名称={selected_template["template_name"]}')
        else:
            # 只有当slide_data中没有模板时才查询数据库
            project_id = slide_data.get('project_id')
            if project_id:
                try:
                    selected_template = await self.get_selected_global_template(project_id, user_id=self.user_id)
                    if selected_template:
                        logger.info(f'✅ [模板加载] 第{page_number}页成功获取选中模板: ID={selected_template["id"]}, 名称={selected_template["template_name"]}')
                        # 缓存到slide_data中，后续重试可以直接使用，不用再次查询
                        slide_data['selected_template'] = selected_template
                    else:
                        logger.warning(f'⚠️ [模板加载] 第{page_number}页未找到选中模板，将使用默认fallback')
                except Exception as e:
                    logger.warning(f'⚠️ [模板加载] 第{page_number}页获取选中模板失败，将使用默认fallback: {str(e)}')
            ...
            try:
                ...
                if validation_result['is_complete']:
                    ...
                    if validation_result['errors']:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            # 优先使用用户选择的模板
                            if selected_template and 'html_template' in selected_template and selected_template['html_template']:
                                logger.info(f'使用选中模板 {selected_template["template_name"]} 作为fallback')
                                fallback_html = selected_template['html_template']
                                title = slide_data.get('title', f'第{page_number}页')
                                content = slide_data.get('content', '')
                                fallback_html = fallback_html.replace('{{title}}', str(title))
                                fallback_html = fallback_html.replace('{{content}}', str(content))
                                fallback_html = fallback_html.replace('{{page_number}}', str(page_number))
                                fallback_html = fallback_html.replace('{{total_pages}}', str(total_pages))
                            else:
                ...
                if 'Expecting value' in error_msg or 'JSON' in error_msg:
                    ...
                    if attempt < max_retries - 1:
                        ...
                        continue
                if attempt == max_retries - 1:
                    logger.error(f'❌ [生成失败] 第{page_number}页所有API重试都失败，启动fallback逻辑')
                    # 优先使用用户选择的模板
                    if selected_template and 'html_template' in selected_template and selected_template['html_template']:
                        logger.info(f'✅ [模板 fallback] 第{page_number}页使用选中模板直接渲染: {selected_template["template_name"]}')
                        fallback_html = selected_template['html_template']
                        title = slide_data.get('title', f'第{page_number}页')
                        content = slide_data.get('content', '')
                        fallback_html = fallback_html.replace('{{title}}', str(title))
                        ...
                        fallback_html = fallback_html.replace('{{page_number}}', str(page_number))
                        fallback_html = fallback_html.replace('{{total_pages}}', str(total_pages))
                        logger.info(f'✅ [模板 fallback] 第{page_number}页模板占位符替换完成，返回HTML')
                    else:
                        logger.warning(f'⚠️ [模板 fallback] 第{page_number}页无可用模板，使用默认fallback模板')
                        fallback_html = self._generate_fallback_slide_html(slide_data, page_number, total_pages)
                    return await self._apply_auto_layout_repair(fallback_html, slide_data, page_number, total_pages)
                continue
        # 优先使用用户选择的模板
        if selected_template and 'html_template' in selected_template and selected_template['html_template']:
            logger.info(f'✅ [模板 fallback] 第{page_number}页生成循环结束，使用选中模板直接渲染: {selected_template["template_name"]}')
            fallback_html = selected_template['html_template']
            title = slide_data.get('title', f'第{page_number}页')
            content = slide_data.get('content', '')
            fallback_html = fallback_html.replace('{{title}}', str(title))
