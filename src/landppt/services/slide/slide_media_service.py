...
class SlideMediaService:
    ...
    async def _generate_single_slide_html_with_prompts(self, slide_data: Dict[str, Any], confirmed_requirements: Dict[str, Any], system_prompt: str, page_number: int, total_pages: int, all_slides: List[Dict[str, Any]]=None, existing_slides_data: List[Dict[str, Any]]=None, project_id: str=None) -> str:
        ...
        try:
            ...
            # If we have a selected template, use it directly instead of generic fallback
            if selected_template and 'html_template' in selected_template and selected_template['html_template']:
                logger.info(f"Falling back to selected template {selected_template['template_name']} after API failure")
                fallback_html = selected_template['html_template']
                # Replace common placeholders in template
