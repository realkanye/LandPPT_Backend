...
class CreativeDesignService:
    ...
    async def _generate_slide_with_template(
        ...
        try:
            ...
            logger.warning(f'⚠️ [模板生成] 第{page_number}页AI生成模板风格失败，启动fallback逻辑')
            # 优先使用当前选中模板直接替换占位符
            if template_html:
                logger.info(f'✅ [模板 fallback] 第{page_number}页使用选中模板直接渲染: {template_name}')
                ...
                logger.warning(f'⚠️ [模板 fallback] 第{page_number}页无可用模板HTML，使用默认fallback模板')
                fallback_html = await self._generate_fallback_slide_html(slide_data, page_number, total_pages)
            return fallback_html
        except Exception as exc:
            logger.error(f'❌ [模板生成] 第{page_number}页使用模板生成幻灯片失败: {str(exc)}', exc)
            # 优先使用当前选中模板直接替换占位符
            if template_html:
                logger.info(f'✅ [模板 fallback] 第{page_number}页使用选中模板直接渲染: {template_name}')
                fallback_html = template_html
    ...
    async def _prepare_project_creative_guidance(
        ...
        if not template_html and project_id:
            try:
                selected_template = await self.get_selected_global_template(project_id, user_id=user_id)
