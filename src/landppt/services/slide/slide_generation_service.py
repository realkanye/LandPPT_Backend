...
class SlideGenerationService:
    ...
    async def _generate_slides_streaming_impl(self, project_id: str):
            ...
            try:
                ...
                # 预加载用户选中的模板，所有幻灯片共享，避免每个幻灯片都查询数据库（防止长项目时数据库连接超时）
                selected_template = None
                try:
                    selected_template = await self.get_selected_global_template(project_id, user_id=self.user_id)
                    if selected_template:
                        logger.info(f'✅ [全局模板预加载] 成功获取项目选中模板: ID={selected_template["id"]}, 名称={selected_template["template_name"]}')
                        # 将模板添加到所有幻灯片数据中，所有幻灯片共享使用，无需重复查询
                        for slide in slides:
                            slide['selected_template'] = selected_template
                    else:
                        logger.warning(f'⚠️ [全局模板预加载] 项目未找到选中模板，将使用默认模板')
                except Exception as e:
                    logger.warning(f'⚠️ [全局模板预加载] 获取项目选中模板失败，将使用默认模板: {str(e)}')
