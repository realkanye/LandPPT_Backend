...
class AuthMiddleware:
    ...
    def __init__(self):
        ...
        self.disable_auth = app_config.disable_auth
        self.anonymous_user_id = app_config.anonymous_user_id
        # 不需要认证的路径
        self.public_paths = {
            "/",
            "/health",
    ...
    async def __call__(self, request: Request, call_next: Callable):
        ...
        try:
            # When auth is disabled, create a dummy anonymous user and skip all auth checks
            if self.disable_auth:
                from ..database.models import User
                # Create a minimal dummy user with the configured anonymous user ID
                # Only set the essential fields that downstream code expects
                class AnonymousUser:
                    def __init__(self, user_id: int):
                        ...
                        self.is_active = True
                        self.is_admin = True
                        self.credits_balance = 999999
                        self.github_id = None
                        self.linuxdo_id = None
                        self.oauth_provider = None
                        self.created_at = 0.0
                        self.last_login = None
                        self.register_ip = None
                        self.last_login_ip = None
                        self.registration_channel = None
                        self.invite_code_id = None
                        self.avatar = None
                ...
                request.state.user = AnonymousUser(self.anonymous_user_id)
                current_user_id.set(self.anonymous_user_id)
